      ...
      <div className="w-full h-full bg-blue-300 p-3">
        ...
        <div className="flex h-full bg-neutral-100">
          ...
          <div className="flex-1 p-8">
            ...
            {/* Posts Grid */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mx-36 mt-8">
              {posts.map((post) => (
                <div
                  key={post.id}
                  className="relative cursor-pointer hover:scale-105 duration-300 bg-gray-200 shadow-md overflow-hidden"
                  onClick={() => openPost(post)}
                >
                  <img
                    src={`http://localhost:8000${post.image_url}`} // 서버 URL과 결합
                    alt={post.title}
                    className="w-full aspect-square object-cover border-2 border-blue-300 hover:border-blue-500"
                  />
                  {/* <div className="">
                    <h2 className="absolute bottom-2 left-2 text-xs font-light text-black truncate">
                      {post.title}
                    </h2>
                  </div> */}
                </div>
      ...
      {selectedPost && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="relative bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            {/* Close Button */}
            <button
              className="absolute top-6 right-6 p-2 text-rose-500 hover:text-rose-700 focus:outline-none"
              onClick={closePost}
            >
              ✕
            </button>
            ...
            <div className="w-full font-light text-xs text-neutral-500 flex justify-end">
              <p>post by {selectedPost.user}</p>
            </div>
          </div>
        </div>
      )}
