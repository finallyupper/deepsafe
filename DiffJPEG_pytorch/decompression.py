...
def y_dequantize(image, factor=1):
    ...
    """
    return image * (utils.y_table.to(image.device) * factor)
...
def c_dequantize(image, factor=1):
    ...
    """
    return image * (utils.c_table.to(image.device) * factor)
...
def idct_8x8(image):
    ...
    """
    alpha = np.array([1. / np.sqrt(2)] + [1] * 7)
    alpha = torch.tensor(alpha, dtype=torch.float32, device=image.device)  
    ...
    for x, y, u, v in itertools.product(range(8), repeat=4):
        tensor[x, y, u, v] = np.cos((2 * u + 1) * x * np.pi / 16) * np.cos(
            (2 * v + 1) * y * np.pi / 16)
    tensor = torch.tensor(tensor, dtype=torch.float32, device=image.device) 
...
def ycbcr_to_rgb_jpeg(image):
    ...
    result = torch.tensordot(image + shift, torch.from_numpy(matrix).to(image.device), dims=1)
    #result = torch.from_numpy(result)
    result.view(image.shape)
    return result.permute(0, 3, 1, 2)


def decompress_jpeg(y, cb, cr, height, width, factor=1):
