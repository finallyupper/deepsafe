...
def train(configs, 
    ...
    #    if epoch == epochs - 1:
        if epoch % 5 == 0:
            ...
            plt.legend()
            plt.savefig(os.path.join(save_path,'loss', "loss_acc.png"))
            plt.close()
            ...
            plt.savefig(os.path.join(save_path,'loss', "loss_img.png"))
            plt.legend()
            plt.close() 

            plt.plot(np.arange(len(train_gan_loss_plot)), train_gan_loss_plot, label="train_gan_loss_plot")
            plt.savefig(os.path.join(save_path,'loss', "loss_gan.png"))
            plt.legend()
            plt.close()

            plt.plot(np.arange(len(train_ssim_loss_plot)), train_ssim_loss_plot, label="train_ssim_loss_plot")
            plt.savefig(os.path.join(save_path,'loss', "loss_ssim.png"))
            plt.legend()
            plt.close()

            plt.plot(np.arange(len(train_ed_loss)), train_ed_loss, label="train_ed_loss")
            plt.plot(np.arange(len(train_adv_loss)), train_adv_loss, label="train_adv_loss")
            plt.plot(np.arange(len(train_loss_plot)), train_loss_plot, label="train_message_loss") 
            plt.savefig(os.path.join(save_path,'loss', "loss_enc_msg.png"))
            plt.legend()
            plt.close()
