...
# Local
import DiffJPEG_pytorch.utils as utils
...
def rgb_to_ycbcr_jpeg(image):
    ...
    image = image.permute(0, 2, 3, 1)
    device = image.device
    result = torch.tensordot(image, torch.from_numpy(matrix).to(device), dims=1) + shift.to(device)
#    result = torch.from_numpy(result)
    result.view(image.shape)
    return result
...
def dct_8x8(image):
    ...
    Input:
        image(tensor): batch x height x width (device에 위치)
    Output:
        dcp(tensor): batch x height x width
    """
    image = image - 128
    ...
    for x, y, u, v in itertools.product(range(8), repeat=4):
        tensor[x, y, u, v] = np.cos((2 * x + 1) * u * np.pi / 16) * np.cos(
            (2 * y + 1) * v * np.pi / 16)
    tensor = torch.tensor(tensor, dtype=torch.float32, device=image.device)  # GPU로 이동
    alpha = torch.tensor([1. / np.sqrt(2)] + [1] * 7, dtype=torch.float32, device=image.device)
    scale = torch.outer(alpha, alpha) * 0.25  # NumPy 대신 PyTorch의 `torch.outer` 사용
...
def y_quantize(image, rounding, factor=1):
    ...
    Output:
        image(tensor): batch x height x width
    """
    image = image.float() / (utils.y_table.to(image.device) * factor)
...
def c_quantize(image, rounding, factor=1):
    ...
    Output:
        image(tensor): batch x height x width
    """
    image = image.float() / (utils.c_table.to(image.device) * factor)
    image = rounding(image)
    return image


def compress_jpeg(imgs, rounding=torch.round, factor=1, device=None):
    ...
    for k in components.keys():
        ...
        comp = dct_8x8(comp)
        comp = c_quantize(comp, rounding, factor=factor) if k in (
            'cb', 'cr') else y_quantize(comp, rounding, factor=factor)
